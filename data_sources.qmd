...
All stations were manually located in a Geographic Information System (GIS) using the relevant Soviet Military Topographic maps (1:200'000) from the corresponding region. The maps were downloaded from <https://maps.vlasenko.net> and subsequently georeferenced in QGIS with the `Raster/Georeferencr` tool there [@QGIS_software]. Where possible, high-resolution satellite imagery from Microsoft® Bing™ Maps was finally used to identify measurement bridges and for a final validation of the identified locations.  

The upstream catchments for each gauge were then mapped in R using the [WhiteboxTools v.2.0.0](https://cran.r-project.org/package=whitebox){target="_blank"}. Like this, we have compiled a list of 297 gauges with their contributing catchments. @fig-gauges-and-basins-ca shows the result.
...
# Gauges and basins
data_path <- 
    "../HydrologicalModeling_CentralAsia_Data/REFERENCE_CaseStudyPacks/COMPLETE_FINAL_BASIN_DATA/"
basins <- readRDS(paste0(data_path,"basins_complete.rds"),) %>% arrange(desc(area_km2)) # ensures that small basins are plotted on top of larger basins
gauges <- readRDS(paste0(data_path,"gauges_complete.rds"))
# kick out NA data
basins <- basins %>% na.omit()
gauges <- gauges[!is.na(gauges$Q_NORM_M3S),]

# Rivers
data_path_grdc <- "../HydrologicalModeling_CentralAsia_Data/REFERENCE_CaseStudyPacks/GRDC"
rivers_1 <- st_read(file.path(data_path_grdc,
                    "wmobb_rivnets_Q09_10_ca_large_river_basins.shp"), quiet = TRUE)
rivers_2 <- st_read(file.path(data_path_grdc,
                    "wmobb_rivnets_Q08_09_ca_large_river_basins.shp"), quiet = TRUE)
rivers_3 <- st_read(file.path(data_path_grdc,
                    "wmobb_rivnets_Q07_08_ca_large_river_basins.shp"), quiet = TRUE)
```

```{r}
#| warning: false
#| echo: false
#| label: fig-gauges-and-basins-ca
#| fig.cap: "Mapped gauges are shown as red dots, the corresponding upstream catchments are colored according to their larger basin they belong to. In total, 297 gauges and catchments were mapped like this." 
...
tm_basemap("Esri.WorldPhysical") +
  ...
  tm_shape(basins) +
    tm_polygons("BASIN",alpha = 0.5) +
  tm_shape(gauges) +
    tm_symbols(col = "red", size = .4, scale = .35)
```

@fig-specific-discharge-mountain-catchments shows the specific discharge mapped onto the catchments of each of the 297 gauges.

```{r}
#| warning: false
#| echo: false
#| label: fig-specific-discharge-mountain-catchments
#| fig.cap: "The specific discharge Q in milimeters [mm] is shown for each of the 297 derived catchments. Blue indicate high values whereas light and pale blue colors indicate very low specific discharge. The geographic distribution of the main runoff contributing areas are clearly visible."
tmap_mode("view")
tm_basemap("Esri.WorldPhysical") +
    tm_shape(rivers_1) +
  tm_lines(col = "blue", size = 1) +
  ...
  tm_lines(col = "blue", size = .333) +
  tm_shape(basins) +
    tm_polygons("q_mm", alpha = 1, palette = "Blues", style = "fisher", title = "Q [mm]") +
  tm_shape(gauges) +
    tm_symbols(col = "red", size = .4, scale = .35)
```

For all basins, mean statistics were computed from the digital elevation model, the land cover data, from climatological and glacier data as well as from 1979 through 2012 for mean average monthly temperature and precipitation and finally, the montly snow cover fraction derived from MODIS Terra and Aqua Snow Cover Data Product which is available from 2001 through end of 2021.

The norm discharge data presented here are used for the long-term water balance modeling presented in @sec-budyko-modeling in Part III of the book. For selected stations and river basins, we have time series available shich can be used for model calibration of time-dependent models. Somce of these data and their processing will be presented and discussed in @sec-discharge-station-data. 
