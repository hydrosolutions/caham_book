...
::: callout-tip
If you want to experiment with the R code yourself, the data for this Chapter can be downloaded [here](https://www.dropbox.com/sh/9z3dafl9h1v1o8j/AACeBP2MiFJyNxVIjido83wDa?dl=1) (Note: size of data is \> 500 MB). You can store these data locally and then perform the analysis on your local computer by correspondingly adjusting the data path in the code block below before loading the data.
:::
...
This Section available data to characterize the Chirchik River Basin from the hydro-climatological perspective. Data access and modeling is further described in the Section on Data @sec-part-ii-data-sources-retrieval-and-preparation in Part II and Part III @sec-hydraulic-hydrological-modeling of this Book.

The available discharge data is shown in @fig-chirchick-river-available-data. These are near complete historic records. See above @fig-chirchik-river-basin-overview for the station locations. Note that all available data has been converted to monthly time series for plotting.
...
The discharge measurements at Gazalkent gauge (upper right panel, shown in red color) started already in 1900. It is one of the longest complete records available in Central Asia. The monthly record of the station is shown in @fig-gazalkent-discharge-time-series. You can zoom into the time series and investigate it in detail.
...
The discharge seasonality of the gauging stations downstream of Charvak reservoir are shown below. Note that we only have monthly values for Ugam station which explains the appearance of the weekly plot in the upper right panel of @fig-chirchik-seasonality-small-tributaries.
...
pQ1 <- q1Q  |> 
  ...
  plot_time_series(date,value,name,
      ...
      .x_lab = "Date", .y_lab = "Discharge [m3/s]", .legend_show = TRUE
      )
pQ2 <- q2Q  |> 
  na.omit() |>
  plot_time_series(date,value,name,
      ...
      .x_lab = "Date",.y_lab = "Discharge [m3/s]", .legend_show = FALSE
      )
pQ3 <- q3Q  |> 
  na.omit() |> 
plot_time_series(date,value,name,
...
![Changes in fractional snow cover in the in the Western Tien Shan mountains, including the Chirchik River basin. Shown are percentage changes over the period 1980 - 2011. Changes in snow cover fraction was computed using CHELSA V21 [@karger_2017] daily precipitation and temperature with a liquid-solid temperature threshold value of 1 deg. Celsius. Source: hydrosolutions GmbH.](/images/example_river_basins/snowcover_fraction_change_chirchik_lr.jpg){#fig-snow-cover-fraction-change-chirchik-river-basin}
