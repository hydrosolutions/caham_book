...
In QGIS, there are several methods that can fill terrain depressions. We are using the `r.fill.dir` algorithm to perform this task. @fig-fill-sinks shows the process in detail. First, we make sure that we have loaded the DEM in the correct projection. Second, we open the Processing Toolbox and type `r.fill.dir` in the search bar, and then double-click on the corresponding algorithm. Third, in the openend window, we ensure that the DEM is selected under the *Elevation* header. On top of that, the *Flow Direction* and *Problem Areas* output files are not needed, and their checkboxes can be left unchecked.
