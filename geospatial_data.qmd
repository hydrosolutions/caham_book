...
Now we can clean up the layer and merge it with the HBV elevation bands layer we prepared and saved earlier. You may want to edit the names of the features for the GSM objects with a prefix *glacier* or *gl* so that you recognize which hydrological model to assign to which HRU in RS MINERVE (see @sec-RSM-automatic-model-generation). You then merge the two vector layers and are ready to import them into RS MINERVE for automated model generation. We save the layer under the name 16297_Chatkal_RSM_HRU.   
