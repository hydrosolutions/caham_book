...
# See section on snow and glacier data for the code snippet on how to extract 
# the glacier thickness data. To extract the data for the entire Central Asian 
...
@fig-glacier-varea-volume-scaling shows the glacier volume estimate with different methods vs. the glacier area. For small glaciers with an area $< 10 km^2$ Erasov (@eq-erasov) performs better than the newly fitted data from RGI and Farinotti (@eq-volume-rgif). For glaciers larger than $10 km^2$ area, the newly fitted function `glacierVolume_RGIF` should be used. 

$$
V_{\text{RGIF}}= a \cdot A^{b}
$$ {#eq-volume-rgif}

With $A$ being the glacier area in $km^2$, $V_{\text{RGIF}}$ beging the glacier volume in $km^3$, and the parameters $a=$ `r as.numeric(round(nlVAw$m$getPars()[1], digits = 3))` and $b=$ `r as.numeric(round(nlVAw$m$getPars()[2], digits = 3))`. The newly fitted scaling relationship is implemented as `glacierVolume_RGIF` in the package `riversCentralAsia`, the Erasov scaling relationship is implemented as `glacierVolume_Erasov`. 

The area-volume scaling by Erasov and with updated parameters (RGIF) can be reversed. Thereby it is possible to update the glacier area as a function of the glacier volume. 

$$
A_{Erasov} = log\big( \big)
$$ {#eq-area-erasov}
...
:::{.callout-warning}
All required data is available from [the downloadable data package](https://www.dropbox.com/sh/mfm3lk2av1t74bw/AABw5ER3E3JOqjp-SdWfLol0a?dl=1){target="_blank"}. Note however that this folder required 12 GB of local storage space!
:::
