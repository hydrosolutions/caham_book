...
@fig-glacier-varea-volume-scaling shows the glacier volume estimate with different methods vs. the glacier area. For small glaciers with an area of $< 10 km^2$ approximately, Erasov (@eq-erasov) performs better than the newly fitted data from RGI and Farinotti (@eq-volume-rgif). For glaciers larger than approximately $10 km^2$ area, the newly fitted function `glacierVolume_RGIF` should be used. 
