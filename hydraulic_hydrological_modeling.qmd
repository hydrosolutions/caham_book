...
All the sub-basins should now display the red HBV icon as shown in @fig-rsm-auto-model-creation-03. If you have glaciers in your basin and prepared your subbasins layer accordingly (see @sec-preparation-of-rsminerve-input-files), you now select the glacier HRUs and allocate GSM objects to them. 
...
Object_IDs <- st_read("../caham_data/SyrDarya/Atbashy/GIS/16076_HRU.shp") |> 
  st_drop_geometry() 

gsm_objects <- Object_IDs |> 
  dplyr::filter(str_detect(name, "Gl"))

data <- tibble(
  Model = rep("Model chon_kemin_model", (length(gsm_objects$name))*2), 
...
writeSelectionCHK(filepath = "../RSMINERVE/HQ_glaciers.chk", 
                  data = data, 
                  name = "HQglaciers")
```
