...
The cryosphere encompasses solid water andis a water balance component of considerable importance in hydrological systems in Central Asia.

The following section gives a brief overview over the available regional open source data regarding Central Asias cryosphere. A later chapter on [Glacier Modeling](@sec-glacier-modeling) will then focus on the modelling options of the cryosphere.
...
Among other things, climate impacts translate into long-term changes of runoff formation fractions and the distribution of runoff formation within the hydrological year. Figure shows a simplified illustration of the changes in runoff regime as glaciers shrink (for the example of a heavily glaciated basin) [@hock_ipccHMA_2019]. The considerable reduction in river discharge as glaciers disappear from the catchment is noteworthy, especially in the late summer period which coincides with a period of high irrigation demand in Central Asia. Further, variability of discharge is expected to increase; with little to no discharge in late summer when glacier storage is gone and occasional high precipitation runoff due to high intensity rainfall events.

![Illustration of the development of glacier melt in a heavily glaciated catchment (Image source: Hock an colleagues: High Mountain Areas. In: IPCC Special Report on the Ocean and Cryosphere in a Changing Climate)](images/glacier_data_section/IPPC_special_report_HighMountainAreas_2019.png)
...
We start with the upper-most cryospheric layer: the snow cover. A large amount of data sets are available and the following section presents a suitable product for the calibration of the snow module in the HBV model objects.

### High Mountain Asia Snow Reanalysis (HMASR) Product
...
For operational hydrological modelling, the MODIS snow covered area product has to be used (see for example Modsnow).

## Glacier outlines #sec-glacier-outlines

The Randolph Glacier Inventory (RGI) v6.0 [@rgi60] makes a consistent global glacier data base publicly available. It includes geo-located glacier geometry and some additional parameters like elevation, length, slope and aspect. A new version (v7) is under review at the time of writing (September 2022). For Central Asian water resources modelling, RGI regions 13 (Central Asia) and 14 (South Asia West) are relevant. The glacier geometries for all RGI regions are available from [the GLIMS RGI v6.0 web site](https://www.glims.org/RGI/rgi60_dl.html){target="_blank"}. For this demo, the data for the Atbashy basin is available from the data download link given above.
